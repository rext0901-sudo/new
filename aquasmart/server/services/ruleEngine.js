...
const { getCurrentPrice, arePricesFresh, getPricesForDate } = require('./entsoeService');
const aquareaService = require('./aquareaService');

let lastActionState = null; // track last applied state to ensure idempotency
...
function getPricePercentileToday() {
  const prices = getPricesForDate(getTodayDateStr());
...
function evaluateCondition(condition, context) {
  ...
  switch (comparator) {
    ...
    case 'between': {
      ...
      // Handle wrap-around for hour_of_day (e.g., 22-06)
...
function buildActionStateKey(action) {
  return `${action.type}:${action.value ?? ''}`;
}

async function executeAction(action) {
  const stateKey = buildActionStateKey(action);

  // Idempotency: skip if we already applied this exact state
  if (lastActionState === stateKey) {
    return { executed: false, reason: 'Already in target state' };
  }
...
async function evaluateRules() {
  // Guard: check if Panasonic API is reachable
  if (!aquareaService.isApiReachable()) {
    console.log('[RuleEngine] Panasonic API unreachable — skipping evaluation');
    return { evaluated: 0, triggered: null, reason: 'API unreachable' };
  }

  // Guard: check if prices are fresh
  if (!arePricesFresh()) {
    console.log('[RuleEngine] Prices stale (>1 hour) — skipping price-based rules');
  }

  const rules = getEnabledRules(); // sorted by priority DESC
  const currentPrice = getCurrentPrice();
  const deviceStatus = aquareaService.getStatus();
  const pricesFresh = arePricesFresh();
  ...
  const context = {
    ...
    price_percentile_today: getPricePercentileToday()
  };

  console.log('[RuleEngine] Context:', JSON.stringify(context));

  for (const rule of rules) {
    // Skip price-based rules if prices are stale
    ...
    if (matches) {
      console.log(`[RuleEngine] Rule "${rule.name}" triggered`);
...
function getActiveRule() {
  if (!lastActionState) return null;
